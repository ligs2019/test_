...
class boss():
    def qingqiu(self,page=1):
        url=url='https://www.zhipin.com/c101280100/' \
                '?query=%E8%BD%AF%E4%BB%B6%E6%B5%8B%E8%AF%95%E5' \
                '%B7%A5%E7%A8%8B%E5%B8%88&page={1}&ka=page-{1}'
        head = {'User-Agent': 'Mozilla/5.0(WindowsN'
               'T10.0;WOW64;rv:66.0)Gecko/20100101Firefox/66.0'}
...
for q,w,e,r,t,y in tt:
    ...
    conn.commit()
    mycusor.execute('select count(*) from boss')
    gg=mycusor.fetchall()
    pp=gg[0][0]
    if int(pp)%30==0:
        fjr='15903893872@163.com'
        sjr='969598484@qq.com'
        server='smtp.163.com'
        passwd='lgs418710'
        ...
        mes.attach(txt)
        smtp666=smtplib.SMTP_SSL(server,465)
        smtp666.login(fjr,passwd)
        smtp666.sendmail(fjr,sjr,mes.as_string())
